...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.buttonnewpage = customtkinter.CTkButton(self,text="Alerte",command=lambda path="/home/test/FIN/graph2.py": self.execute_script(path))
       ...
       #Test 1 : SPAM (dans le cas ou  1 même mac demande plusieur adresse ip)
        ...
        #self.ajouter_element_arborescence("192.168.1.2", "192.168.1.2","DHCPDECLINE","10.10.10.10","macdst","Informations 1","time")
        #self.ajouter_element_arborescence("192.168.1.3", "192.168.1.2","DHCPREQUEST","11.10.10.10","macdst","Informations 1","time")
        #self.ajouter_element_arborescence("192.168.1.4", "192.168.1.2","DHCPREQUEST","11.10.10.10","macdst","Informations 1","time")
        #self.ajouter_element_arborescence("192.168.1.3", "192.168.1.2","DHCPREQUEST","10.10.11.10","macdst","Informations 1","time")
        #self.ajouter_element_arborescence("192.168.1.4", "192.168.1.2","DHCPREQUEST","10.11.10.10","macdst","Informations 1","time")
        # Test2 : 
        self.timestamps = []
    ...
    def sidebar_button_event_start(self):
        ...
        #loop.run_until_complete(watch_collection(self))  
    ...
    def affiche_paquet(self,paquet):
        current_time, ip_src, ip_dst, type_paquet, mac_src, mac_dst, paquet = transfert_interface(paquet)
        self.ajouter_element_arborescence(ip_src, ip_dst, type_paquet, mac_src,mac_dst,paquet,current_time)
...
def listen_to_changes(app):
    client = MongoClient('mongodb://root:password@192.168.39.209:27017')
    db = client['data']
    collection = db['packet_dhcp']
    # Création d'un change stream
    change_stream = collection.watch(full_document='updateLookup')

    # Écoute des nouveaux changements
    for change in change_stream:
        change = change['fullDocument']
        # Traitement des changements
        print('Received a change: ', change)
        app.affiche_paquet(change)

t = threading.Thread(target=listen_to_changes, args=(app,))
t.start()
...
# Exécuter la fonction watch_collection
