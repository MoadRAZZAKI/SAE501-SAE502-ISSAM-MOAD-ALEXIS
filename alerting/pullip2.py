...
def calculate_ips():
    ...
    try:
        ...
        count = len(ip_list)
        label_count.config(text=f"Taille de la liste: {count}")
...
label_remove_ip = tk.Label(root, text="Adresse IP à supprimer temporairement:")
label_remove_ip.grid(row=4, column=0, padx=5, pady=5, sticky="e")

entry_remove_ip = tk.Entry(root)
entry_remove_ip.grid(row=4, column=1, padx=5, pady=5)

button_remove_temporarily = tk.Button(root, text="Supprimer temporairement", command=remove_temporarily)
button_remove_temporarily.grid(row=5, column=0, columnspan=2, padx=5, pady=5)

# Label to display the size of the list
label_count = tk.Label(root, text="Taille de la liste: 0")
